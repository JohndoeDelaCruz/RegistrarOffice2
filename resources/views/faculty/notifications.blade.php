...
<div class="bg-white rounded-lg shadow-sm p-6 mb-6">
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center">
        <div>
            <h1 class="text-2xl font-bold text-gray-800 mb-2">Deadline Notifications</h1>
            <p class="text-gray-600">Grade completion deadline reminders and alerts</p>
        </div>
...
<div class="bg-white rounded-lg shadow-sm overflow-hidden">
                    ...
                    <div class="flex items-start space-x-4 flex-1">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center space-x-2 mb-2">
                                ...
                                @if($notification->days_until_deadline > 0)
                                    @php
                                        $badgeClass = '';
                                        if ($notification->days_until_deadline <= 1) {
                                            $badgeClass = 'bg-red-100 text-red-800';
                                        } elseif ($notification->days_until_deadline <= 3) {
                            ...
                            <div class="bg-gray-50 rounded-lg p-4 mb-3">
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-900 font-medium">{{ $notification->application->student->name }}</p>
                                        <p class="text-xs text-gray-500">{{ $notification->application->student->student_id }}</p>
                                    </div>
                                    <div>
                                        <span class="text-sm font-medium text-gray-600">Subject:</span>
                                        <p class="text-sm text-gray-900 font-medium">{{ $notification->application->subject->code }}</p>
                                        <p class="text-xs text-gray-500">{{ $notification->application->subject->description }}</p>
                                    </div>
                                    <div>
                                        <span class="text-sm font-medium text-gray-600">Deadline:</span>
                                        <p class="text-sm text-gray-900 font-medium">{{ $notification->deadline_date->format('M j, Y') }}</p>
                                        <p class="text-xs text-gray-500">{{ $notification->deadline_date->format('l') }}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center justify-between text-sm text-gray-500">
                                ...
                                @if($notification->is_read && $notification->read_at)
                                    <span class="text-xs">Read {{ $notification->read_at->diffForHumans() }}</span>
                                @endif
                            </div>
                        </div>
                    </div>
    ...
    @else
        ...
        <div class="text-center py-12">
            ...
            <p class="text-gray-500 mb-6">You don't have any deadline notifications at the moment.</p>
...
function markAsRead(notificationId) {
    ...
    .then(data => {
        if (data.success) {
            ...
            // Add read timestamp
            const timestampDiv = notification.querySelector('.text-sm.text-gray-500');
            if (timestampDiv && !timestampDiv.textContent.includes('Read')) {
                timestampDiv.innerHTML += '<span class="text-xs ml-4">Read just now</span>';
            }
        } else {
